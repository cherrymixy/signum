...
import { DecodingAnalysisNodeData, AnalysisResult, AnalysisItem } from '@/types';
...
export default function DecodingAnalysisNode({ data, type }: DecodingAnalysisNodeProps) {
  ...
  const handleAnalyze = useCallback(async () => {
    if (!data.connectedImageNodeData?.imageBase64 || !data.connectedImageNodeData?.imageMimeType) {
      data.onUpdate({
        status: 'error',
    ...
    try {
      const response = await analyzeImage({
        imageBase64: data.connectedImageNodeData.imageBase64,
        imageMimeType: data.connectedImageNodeData.imageMimeType,
        intentText,
        targetPreset,
        contextPreset,
      });
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg shadow-[0_1px_3px_rgba(0,0,0,0.3)] min-w-[360px] max-w-[480px]">
      ...
      <div className="p-6 space-y-5">
        ...
        {!data.connectedImageNodeData?.imageBase64 && (
